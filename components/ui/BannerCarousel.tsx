    ...
    >
      ...
      {action && (
        <div class="absolute top-0 bottom-0 m-auto left-0 right-0 sm:right-auto sm:left-[12%] max-h-min max-w-[235px] flex flex-col gap-4 p-4 rounded glass">
          <span class="text-3xl text-zinc-700 font-medium text-base-100">
            {action.title}
          </span>
          <span class="font-medium text-zinc-700 text-xl text-base-100">
            {action.subTitle}
          </span>
          <Button class="glass text-zinc-600">{action.label}</Button>
        </div>
      )}
    </a>
  );
